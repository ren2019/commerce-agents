...
这是由演示者操作的本地案例，供现场展示或自行录屏。沿用官方 retail 顾客与商家故事；订单、支付与经营数据均为模拟，不开放客户自由体验。自动与代理浏览器验证已完成，仍需演示者亲自操作复核后才能标记为可录制验收完成。
...
浏览器实测偶发首次请求连接失败。聊天失败时检查服务并重试；批准请求失败时先看卡片和近期变更记录，界面会回查同一变更是否已执行，不能把错误提示当作未执行的证明。不要重新生成同一补货提案来绕过核查。小样例仍沿用官方首页提问按钮，换行业后请按客户数据使用自己的演示脚本。
