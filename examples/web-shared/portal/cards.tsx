...
export function PriceBand({ current, floor, ceiling }: { current: number; floor: number; ceiling: number }) {
  ...
  return (
    <div className="relative mt-1 h-[54px]" role="img" aria-label={language === "zh" ? `当前价格${formatMoney(current)}，下限${formatMoney(floor)}，上限${formatMoney(ceiling)}` : `${formatMoney(current)} now, floor ${formatMoney(floor)}, ceiling ${formatMoney(ceiling)}`}>
