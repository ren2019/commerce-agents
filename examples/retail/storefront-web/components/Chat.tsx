...
import { useDemoLanguage, ActivityLine, type AgentTurn, type AssistantChatItem, Chat as ChatShell } from "web-shared";
...
function Pending({ item }: { item: AssistantChatItem }) {
  ...
  return (
    <section role="status" className="rounded-2xl border border-(--line) bg-(--card) p-3 shadow-(--shadow-sm)">
      <div className="mb-3 animate-pulse text-[15px] text-(--ink-soft)">{t(item.activity ?? "Searching the catalog…")}</div>
