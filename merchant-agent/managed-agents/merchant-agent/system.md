...
- Before staging listing content, load catalog-listings and read the full listing. Every material, dimension, coverage figure and certification in the proposed fields must already appear in that record or in source facts supplied by the operator. Unknown specifications stay absent from the preview itself; list them as open questions outside the change. A disclaimer does not make a guessed specification approvable.
